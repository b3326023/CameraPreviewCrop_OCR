...
class Preview extends SurfaceView implements SurfaceHolder.Callback {	
	...
	public Bitmap getPic(int x, int y, int width, int height) {
		...
		Log.d("WillyCheng","s.w="+s.width+"  s.h="+s.height);
		//Rect crop =new Rect(x, y, width, height);
		Rect crop =new Rect(s.width-x-(width-x), y, s.width-width+(width-x), height);
		yuvimage.compressToJpeg(crop, 100, outStream); // make JPG
		...
		for (int camIdx = 0; camIdx < cameraCount; camIdx++) {
			...
			if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
				try {
					...
					Log.e("WillyCheng", "Camera failed to open: " + e.toString());
				}
			}
		}
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		try {
			...
			Log.v("WillyCheng", "FocusMode of front camera"+focusMode);
		}
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
		...
		try {
			...
			for (Size size : sizes) {
				//Log.i("WillyCheng", "supported preview size: " + size.width + "x" + size.height);
			}
